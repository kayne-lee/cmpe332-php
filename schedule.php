    ...
    <body class="relative bg-gray-100 min-h-screen font-mono">
        ...
        <div class="p-40">
            <?php
                ...
                $selectedDate = $_POST["dateOfEvent"];

                echo "<h1 class='text-2xl font-bold mb-6'>Sessions on ".$selectedDate.":</h1>";

                $query = 'SELECT location, startTime, endTime FROM sessions WHERE dateOfEvent="'.$selectedDate.'" ORDER BY startTime';
                $result = $connection->query($query);
            ?>

            <table class="w-full max-w-3xl border border-gray-400 bg-white rounded shadow">
                <thead>
                        ...
                        if ($result->rowCount() > 0) {
                            while ($row = $result->fetch()) {
                                echo "<tr>
                                        ...
                                        <td class='p-2 border'>".$row['startTime']."</td>
                                        <td class='p-2 border'>".$row['endTime']."</td>
                                    </tr>";
                            }
                        } else {
                            echo "<tr><td colspan='3' class='p-4 text-center text-gray-500'>No sessions found for this date.</td></tr>";
                        }
